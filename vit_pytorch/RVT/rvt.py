...
class PoolingTransformer(nn.Module):
    def __init__(self, image_size,loss_type, GPU_ID, patch_size, stride, base_dims, depth, heads,
        ...
        if num_classes > 0:
            ...
            if self.loss_type == 'None':
                ...
                if self.loss_type == 'Softmax':
                    self.head = Softmax(in_features=base_dims[-1] * heads[-1], out_features=num_classes, device_id=self.GPU_ID)
                elif self.loss_type == 'CosFace':
                    self.head = CosFace(in_features=base_dims[-1] * heads[-1], out_features=num_classes, device_id=self.GPU_ID)
                elif self.loss_type == 'ArcFace':
                    self.head = ArcFace(in_features=base_dims[-1] * heads[-1], out_features=num_classes, device_id=self.GPU_ID)
                elif self.loss_type == 'SFace':
                    self.head = SFaceLoss(in_features=base_dims[-1] * heads[-1], out_features=num_classes, device_id=self.GPU_ID)
        else:
    ...
    def get_classifier(self, label=None):
        emb = self.mlp_head
        if label is not None:
            x = self.head(emb, label)
            return x, emb
        else:
            return emb
