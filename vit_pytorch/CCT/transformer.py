...
class Attention(Module):
    ...
    def forward(self, x, mask=None):
        ...
        if mask is not None:
            ...
            assert mask.shape[-1] == attn.shape[-1], 'mask has incorrect dimensions'  # Something is changing here
...
class TransformerEncoderLayer(Module):
    ...
    def forward(self, src: torch.Tensor, mask=None, *args, **kwargs) -> torch.Tensor:
        src = src + self.drop_path(self.self_attn(self.pre_norm(src)),mask=mask)
        src = self.norm1(src)
