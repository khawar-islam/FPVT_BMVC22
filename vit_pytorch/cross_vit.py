...
class Attention(nn.Module):
    ...
    def forward(self, x, context=None, kv_include_self=False, mask=None):
    ...
    def __init__(
        ...
        if self.loss_type == 'None':
            ...
            if self.loss_type == 'Softmax':
                self.loss = Softmax(in_features=sm_dim, out_features=num_classes, device_id=self.GPU_ID)
            elif self.loss_type == 'CosFace':
                self.loss = CosFace(in_features=sm_dim, out_features=num_classes, device_id=self.GPU_ID)
            elif self.loss_type == 'ArcFace':
                self.loss = ArcFace(in_features=sm_dim, out_features=num_classes, device_id=self.GPU_ID)
            elif self.loss_type == 'SFace':
                self.loss = SFaceLoss(in_features=sm_dim, out_features=num_classes, device_id=self.GPU_ID)
